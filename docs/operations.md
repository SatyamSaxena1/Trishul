...
For a quarterly sandbox drill, record the start time, copy `recovery.backend.hcl.example` and `recovery.tfvars.example`, fill their non-secret identifiers from the primary Terraform outputs, and initialize a separate Terraform data directory and backend key. Apply `deployment_mode=recovery`; it reuses the replica bucket, restores RDS to the latest replicated point, and creates regional EKS, Valkey, and CodeBuild only for the drill. Sync the replicated runtime secret through External Secrets, configure the protected GitHub environment with the recovery CodeBuild outputs, and deploy the exact tested commit.
