...
Apply [`checkpoint/`](checkpoint/) in the security account first, then copy its bucket name and account ID into this root. The hourly AWS-overlay CronJob can only create checkpoint object versions and emit its success metric; it has no checkpoint read, list, or delete permission, and Object Lock prevents mutation of existing versions.
...
Primary mode also creates a destination-region KMS key and evidence replica with S3 RTC, RDS automated-backup replication, Secrets Manager replication, and recovery-point alarms. No DR EKS or Valkey runs until recovery mode is applied with a separate backend key:

```sh
export TF_DATA_DIR=.terraform-recovery
cp recovery.backend.hcl.example recovery.backend.hcl
cp recovery.tfvars.example recovery.tfvars
terraform init -reconfigure -backend-config=recovery.backend.hcl
